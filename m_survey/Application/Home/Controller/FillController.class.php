...
class FillController extends Controller {
    ...
    public function fill()
    {
        ...
        if(!$p)
            $p = 0;
        if($p <= 0)
            $this->redirect("Fill/fill",array('p'=>1));
        elseif($p > $last && $last > 0)
            $this->redirect("Fill/fill",array('p'=>$last));

        $show = $page->show();
        $survey=$link->order('hit desc')->where($map)->limit($page->firstRow.','.$page->listRows)->select();
...
//        echo $survey['anonymity'];
        ...
        if($survey['anonymity'] == 0){
            ...
            if(M('Result')->where($map)->find())
                echo "<script language='javascript'>alert('您已填写该问卷！');javascript:window.location.href='".U('Fill/fill')."'</script>";
    ...
    public function add(){
        ...
        foreach($result as $key => $info){
            ...
            if($data['question_type'] == 1){
               echo $map_op['question_id'] = $key;
               echo $map_op['option_num'] = $info;
                M('Option')->where($map_op)->setInc('sum');
